...
QRectF SimpleIconButtonItem::boundingRect() const {
    return QRectF(QPoint(0, 0), sizeHint());
}
...
bool FileTransferItem::isActive() const {
    return active;
}

void FileTransferItem::onFileTransferUpdate(ToxFile file) {
    updateWidget(file);
}
...
void FileTransferItem::layoutChildren() {
    ...
    // 中心区域
    {
        ...
        // 先排进度条，最下
        {
            qreal height = progressBar->boundingRect().height();
            br.setBottom(br.bottom() - height);
            progressBar->setGeometry(QRectF(br.bottomLeft(), QSize(br.width(), height)));
            br.setBottom(br.bottom() - spacing);
        }
        // 名称与状态均分
        qreal nameHeight = fileNameItem->boundingRect().height();
        qreal statusHeight = fileSizeItem->boundingRect().height();
        qreal space = br.height() - nameHeight - statusHeight - spacing;
        if (space <= 0) {
            fileNameItem->setPos(br.topLeft());

            br.setTop(br.top() + nameHeight + spacing);
            // 重新调整进度条位置
            progressBar->setPos(QPointF(br.left(), br.top() + statusHeight + spacing));

            // 状态区域
            br.setBottom(br.top() + statusHeight);
        } else {
            qreal ave = (br.height() - spacing) / 2;
            // 名称
            {
                qreal y = (ave - nameHeight) / 2;
                fileNameItem->setPos(br.topLeft() + QPointF(0, y));
                br.setTop(br.top() + ave + spacing);
            }
        }
        // 状态区域
        {
            QList<SimpleText*> lables;
            if (fileSizeItem->isVisible())
                lables << fileSizeItem;
            if (progressItem->isVisible())
                lables << progressItem;
            if (etaItem->isVisible())
                lables << etaItem;
...
QRectF FileTransferItem::boundingRect() const {
    return QRectF(QPointF(0, 0), QSize(300, 64));
}
...
void FileTransferItem::onLeftButtonClicked() {
    handleButton(leftButton);
}

void FileTransferItem::onRightButtonClicked() {
    handleButton(rightButton);
}

void FileTransferItem::onPreviewButtonClicked() {
    handleButton(previewButton);
}
